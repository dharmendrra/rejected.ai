...
*   **📝 LLM Call Audit Logging:** Optional `LLM_LOG_LEVEL` (`off` / `info` / `debug`) wraps every Ollama/Anthropic call via a decorator and appends JSON Lines traces (model, prompt/output sizes, latency, errors; full prompts + raw responses at `debug`) to `logs/llm_calls.log`. Documented in `docs/LLM_AUDIT_LOG_FORMAT.md`.
